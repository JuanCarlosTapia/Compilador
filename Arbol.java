...
public class Arbol {
	...
	public Node CrearArbol(ParseTree tree ){
    	...
    	{
    		Node newChild = new Node(tree.getPayload().getClass().toString().replaceAll("Context", "").replaceAll("Hello", "").replaceAll("Parser", "").replaceAll("$", "").replaceAll("class", "").substring(2));
	...
	public void MostrarArbol(Node tree, int nivel, DefaultMutableTreeNode node){
    	...
    	else
    	{
    		...
    		for(int i=0;i<tree.getHijos().size();i++ ){
    			...
    			node.add(new DefaultMutableTreeNode(tree.getHijos().get(i).getData()+" - "+tree.getHijos().get(i).getTipo()));
